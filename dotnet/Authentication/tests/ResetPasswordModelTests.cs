// Hirameku is a cloud-native, vendor-agnostic, serverless application for
// studying flashcards with support for localization and accessibility.
// Copyright (C) 2023 Jon Nicholson
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.

namespace Hirameku.Authentication.Tests;

using Hirameku.TestTools;

[TestClass]
public class ResetPasswordModelTests
{
    private const string Password = TestData.Password;
    private const string RecaptchaResponse = nameof(RecaptchaResponse);
    private const string SerializedToken = TestData.Token;

    [TestMethod]
    [TestCategory(TestCategories.Unit)]
    public void ResetPasswordModel_Constructor()
    {
        var target = GetTarget();

        Assert.IsNotNull(target);
    }

    [TestMethod]
    [TestCategory(TestCategories.Unit)]
    public void ResetPasswordModel_Password()
    {
        var target = GetTarget();

        Assert.AreEqual(Password, target.Password);
    }

    [TestMethod]
    [TestCategory(TestCategories.Unit)]
    public void ResetPasswordModel_RecaptchaResponse()
    {
        var target = GetTarget();

        Assert.AreEqual(RecaptchaResponse, target.RecaptchaResponse);
    }

    [TestMethod]
    [TestCategory(TestCategories.Unit)]
    public void ResetPasswordModel_SerializedToken()
    {
        var target = GetTarget();

        Assert.AreEqual(SerializedToken, target.SerializedToken);
    }

    private static ResetPasswordModel GetTarget()
    {
        return new ResetPasswordModel()
        {
            Password = Password,
            RecaptchaResponse = RecaptchaResponse,
            SerializedToken = SerializedToken,
        };
    }
}
